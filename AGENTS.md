...
- 検討中の課題、実装候補、先送り判断を整理する `docs/backlog/`
  - 参照タイミング: 次に着手する課題を決めるとき、実装方針の候補や未解決論点を確認したいときに参照する。
  - 変更タイミング: 新しい課題や方針候補を追加したとき、優先度・前提・判断内容が変わったときに同じ変更内で更新する。
- 比較調査や検証結果、その根拠データを記録する `docs/survey/`
  - 参照タイミング: 技術選定、品質比較、外部サービス採用判断の根拠を確認したいときに参照する。
  - 変更タイミング: 新しい調査や比較実験を追加したとき、実施条件・観察結果・結論が変わったときに同じ変更内で更新する。
