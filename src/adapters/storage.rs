use crate::{CaptureStore, CaptureStoreError, DiarizedTranscript};
...
mod tests {
    ...
    use crate::{CaptureStore, DiarizedTranscript, TranscriptSegment};

    #[test]
    /// セッション配下に captures ディレクトリと final.jsonl を作成して同じ結果を書き出す。
    fn persists_capture_json_and_appends_final_jsonl_under_session_directory() {
        let temp_dir = tempfile::tempdir().unwrap();
        let mut store = FileSystemCaptureStore::new(temp_dir.path()).unwrap();

        store.persist_capture(1, &sample_transcript()).unwrap();
    ...
    fn names_capture_files_with_zero_padded_sequence() {
        ...
        store.persist_capture(12, &sample_transcript()).unwrap();
